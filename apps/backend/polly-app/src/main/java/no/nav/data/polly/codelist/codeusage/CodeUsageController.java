...
public class CodeUsageController {
    ...
    public ResponseEntity<List<CodeUsageResponse>> findAllCodeUsageOfListname(@PathVariable String list) {
        ...
        List<CodeUsageResponse> responses = service.findCodeUsageOfList(ListName.valueOf(list));
        if (responses.isEmpty()) {
            log.info("The listName {} is never used", list);
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        log.info("Usage of listName {} : {}", list, responses.toString());
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Fetch all usage of requested codelists", response = CodeUsageResponse.class, responseContainer = "List"),
            @ApiResponse(code = 404, message = "Code or listName not found"),
            @ApiResponse(code = 500, message = "Internal server error")})
    @GetMapping("/find")
    public ResponseEntity<List<CodeUsageResponse>> findCodeUsagesByRequests(@RequestBody List<CodeUsageRequest> requests) {
        log.info("Received request to fetch all usage of requested codelists");
        service.validateRequests(requests);

        List<CodeUsageResponse> responses = service.findCodeUsage(requests);
        if (responses.isEmpty()) {
            log.info("The requested codelists are never used. Request: {}", responses.toString());
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        log.info("Usage of the requested codelist: {}", responses.toString());
        return ResponseEntity.ok(responses);
    }
