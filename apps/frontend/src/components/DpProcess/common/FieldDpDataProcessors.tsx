        ...
        <Block width='100%'>
          <Block width='100%'>
            <Select
              ...
              options={props.options.sort((a, b) => (a.label || '').toLocaleString().localeCompare((b.label || '').toLocaleString())).filter(dp => !props.formikBag.values.subDataProcessing.processors.includes(dp.id ? dp.id.toString() : ''))}
              onChange={(params) => {
                if (params.value[0].id && params.value[0].label) {
                  subDataProcessors.set(params.value[0].id.toString(), params.value[0].label.toString())
                }
                arrayHelpers.form.setFieldValue('subDataProcessing.processors', [...props.formikBag.values.subDataProcessing.processors || [], ...params.value.map(v => v.id)])
              }}
            />
          </Block>
          <Block>{props.formikBag.values.subDataProcessing.processors && renderTagList(props.formikBag.values.subDataProcessing.processors.map(dp => {
