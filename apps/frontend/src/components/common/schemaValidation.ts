...
const legalBasesOpenMessage =
  'Du har en pågående redigering av behandlingsgrunnlag. Klikk «Legg til»/«Oppdater» eller Avbryt redigeringen før du lagrer.'
...
export const disclosureSchema: () => yup.ObjectSchema<IDisclosureFormValues> = () =>
  yup.object({
    ...
    processes: yup.array<any>().min(1, requiredMessage).required(requiredMessage),
    abroad: disclosureAbroadSchema().required(),
    processIds: yup.array().of(yup.string().required()).default([]).required(),
    ...
    confidentialityDescription: yup.string().when('assessedConfidentiality', {
      is: (value: unknown) => value === true || value === false,
