        ...
        <Block width='100%'>
          <Block width='100%'>
            <Select
              ...
              options={
                Object.keys(NoDpiaReason).filter((reason) => props.formikBag.values.dpia.noDpiaReasons.indexOf(reason) === -1).map((reason) => {
                  return {
          ...
          <Block>{renderTagList(props.formikBag.values.dpia.noDpiaReasons.map(p => getNoDpiaLabel(p)), arrayHelpers)}</Block>
        </Block>
      </>
    )}
  />
